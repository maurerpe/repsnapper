...
double PLine3::max_abs_speed(double max_Espeed, double max_AOspeed) const
{
  ...
  double t = time();
  if (t != 0) {
    double tot_speed = abs(extrusion+absolute_extrusion) / time();
			...
			const Settings &settings) const
{
  ...
  double comm_speed = speed;

  comm_speed = min(comm_speed, max_abs_speed(maxEspeed, maxAOspeed));

  if (abs(absolute_extrusion) < 0.00001)
    comm_speed = max(minspeed, speed); // in case speed is too low
  ...
  if (abs(dZ) > 0.00001) {
    const double xyztime = travel_length / comm_speed;
    ...
    if ( zspeed > maxZspeed ) {
      ...
      Command preFeedrate(COORDINATEDMOTION, lifted_from, 0, comm_speed);
  ...
  if (arc)
    {
      ...
      command = Command (gc, lifted_to, extrudedMaterial, comm_speed);
    ...
    {
      command = Command (COORDINATEDMOTION, lifted_to, extrudedMaterial, comm_speed);
    }
...
// PLine3old::PLine3old(PLineArea area_,  const uint extruder_no_,
// 	       const Vector3d &from_, const Vector3d &to_,
...
// PLine3old::PLine3old(const PLine &pline, double z)
// {
//   if (pline.lifted > 0) {
//     z += pline.lifted;
//     lifted = true;
...
//   to                 = Vector3d(pline.to.x(),   pline.to.y(),   z);
//   speed              = pline.speed;
//   extrusionfactor    = pline.feedratio;
...
// int PLine3old::getCommands(Vector3d &lastpos, vector<Command> &commands,
// 			double extrusion,
// 			const Settings &settings) const
// {
//   const double
...
//   const double travel_length = length();
//   double extrudedMaterial = travel_length * extrusionfactor * extrusion;
...
//   if (travel_length > 0.00001)
//     espeed = extrudedMaterial*comm_speed/travel_length;
...
// double PLine3old::time() const
// {
//   return length()/speed;
// }

// double PLine3old::length() const
// {
//   if (!arc)
...
double Printlines::total_rel_Extrusion(const vector<PLine2> &lines) const
{
  double l = 0;
  for (lineCIt lIt = lines.begin(); lIt!=lines.end();++lIt){
    l += lIt->length() * lIt->feedratio;
  }
  return l;
}

double Printlines::total_Extrusion(const vector<PLine2> &lines) const
{
  double l = 0;
  for (lineCIt lIt = lines.begin(); lIt!=lines.end();++lIt){
